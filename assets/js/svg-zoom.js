 ...
 * * Works on all Mermaid <svg> elements in the document via Event Delegation.
 */
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /**
   * Helper: Ensure we are interacting with a Mermaid SVG element
   * Explicitly ignores SVG icons inside UI buttons
   */
  const getTargetSvg = (e) => {
    ...
    if (
      ...
      svg.closest(".svg-pan-modal-reset") ||
      svg.closest(".svg-pan-modal-close") ||
      !(svg.id.startsWith("mermaid") || svg.closest(".mermaid") || svg.classList.contains("mermaid"))
    )
      return null;
    return svg;
  };
  ...
  // Hover Logic: Show Expand & Reset Buttons
  document.addEventListener("mousemove", (e) => {
    ...
    // Case 1: Mouse is over an actual SVG
    if (svg) {
      if (
    ...
    // Case 2: Mouse is over the Expand or Reset Button
    else if (overBtn) {
      // Do nothing, just keep buttons visible.
    }
  ...
  // Update button pos on scroll or resize
  const updatePos = () => {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      // Ensure originalViewbox is preserved
      if (!svg.dataset.originalViewbox) {
        const currentVb = svg.getAttribute("viewBox");
        if (currentVb) svg.dataset.originalViewbox = currentVb;
      }
  ...
  document.addEventListener("pointerdown", (e) => {
    ...
    // Don't trigger drag if clicking the expand or reset buttons
    if (
      e.target.closest(".svg-pan-expand-btn") ||
      e.target.closest(".svg-pan-reset-btn") ||
      e.target.closest(".svg-pan-modal-reset") ||
      e.target.closest(".svg-pan-modal-close")
    )
      return;
